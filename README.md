...
Small permissive UTF-8 encode/decode library.
...
- Decodes *any* bytestream, even with invalid unicode codepoints.
- Valid UTF-8 is always decoded to valid unicode codepoints, and valid
  unicode codepoints are always encoded as valid UTF-8.
- Correctly encodes and decodes unsigned integer values up to 31 bits
...
- `int utf8enclen(uint32_t val)`: number of bytes (1..6) to encode `val`, or 0 if `val` > 0x7FFFFFFF
...
- `int utf8encode(const uint32_t *u32, int u32len, char *utf8, int utf8cap)`: encode array to bytes; returns required length even if `utf8cap` is too small; values > 0x7FFFFFFF are silently skipped
